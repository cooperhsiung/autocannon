...
const { PROTOCOL, SERVICE, METHOD, REQUEST } = process.env.CLIENT_PROVIDER ? require(process.env.CLIENT_PROVIDER) : {}
...
Client.prototype._connect = function () {
  if (this.secure) {
    ...
    if (this.ipc) {
      ...
      if (PROTOCOL === 'thrift') {
        this.conn = thrift.createConnection(this.opts.hostname, this.opts.port, {
  ...
  this.conn.on('data', (chunk) => {
    ...
    if (PROTOCOL === 'thrift') {
      this.parser[HTTPParser.kOnHeadersComplete]({ statusCode: 200 })
      this.parser[HTTPParser.kOnBody](chunk, 0, chunk.length)
      this.parser[HTTPParser.kOnMessageComplete]()
    } else {
    ...
    if (PROTOCOL === 'thrift') {
      this.client[METHOD](REQUEST, (err, resp) => {
        if (err) {
          ...
          this.conn.emit('data', JSON.stringify(resp))
        }
      })
    } else {
